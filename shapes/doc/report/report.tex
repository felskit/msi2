...
Ocenie zdecydowano się poddać kilka kryteriów. Wyniki klasyfikacji postanowiono zbadać poprzez wyznaczenie dokładności \emph{(ang. accuracy)}, która jest jedną z miar oceny jakości klasyfikacji. Wydajność klasyfikatorów zweryfikowano mierząc czas wywołań klasyfikacji dla każdego z~nich. Ponadto klasyfikatory poddano próbie bardziej empirycznej, polegającej na teście w czasie rzeczywistym. 

\subsubsection{Dokładność}

Dokładność jest najbardziej intuicyjnym pojęciem pozwalającym na zmierzenie rozumianej przez nas skuteczności klasyfikatora. Dokładność jest bowiem stosunkiem liczby poprawnych klasyfikacji do liczności całej badanej populacji.
...
\end{itemize}
Zdecydowano się nie liczyć mediany, ponieważ wymagałoby to pamiętania wszystkich czasów klasyfikacji, co negatywnie wpłynęło by na zużycie pamięci.

\subsubsection{Próba czasowa}

W tym teście zbadano zachowanie wszystkich trzech jednocześnie w tym samym scenariuszu -- rozpoznawaniu kształtów w czasie rzeczywistym na strumieniu wideo. Wykorzystano w tym celu opisywany wcześniej skrypt \verb+capture.py+, który skonfigurowano na rozpoznawanie konkretnego kształtu i uruchomiono próbę czasową. Zadaniem jest poprawna klasyfikacja pokazywanego kształtu przez jak najdłuższy okres czasu, przy czym dopuszcza się pomyłkę nie dłużej niż 8 klatek z rzędu. W przeciwnym przypadku czas przestaje być mierzony dla danego klasyfikatora.
...
\begin{itemize}
    \item Klasyfikator geometryczny zakwalifikował poprawnie 14911 kształtów, co daje dokładność na poziomie 0.9961 w przybliżeniu. Tym razem najgorzej wypadły trójkąty -- były tam aż~153 pomyłki.
    \item Klasyfikator korzystający z sieci neuronowej klasyfikującej wektory zidentyfikował poprawnie 14813 kształtów, co daje dokładność na poziomie 0.9895 w przybliżeniu. Najwięcej -- bo aż 46 -- błędów zostało popełnionych w przypadku klasyfikacji gwiazd.
    \item Klasyfikator korzystający z konwolucyjnej sieci neuronowej rozpoznał poprawnie 14915 kształtów, co daje dokładność na poziomie 0.9963 w przybliżeniu. Podobnie jak poprzednio, najgorzej wypadły trójkąty, jednak tym razem pomyłek było mniej, bo zaledwie 53.
\end{itemize}

Warto zauważyć, że poza siedmioma przypadkami błędnej identyfikacji trójkątów jako kwadraty i~gwiazdy przez sieć konwolucyjną, nie wystąpiły żadne błędy pierwszego typu \emph{(ang. false positive errors)}. Wszystkie pozostałe błędy dotyczyły nierozpoznania kształtu całkowicie.

Jak już wcześniej zostało wspomniane, dokładność była także wypisywana na ekran w trakcie treningu sieci neuronowych. Wartość wyznaczona podczas ewaluacji wytrenowanego modelu w~przypadku sieci klasyfikującej wektory to 0.9980, natomiast dla sieci konwolucyjnej zwrócona wartość była 1.0 (wyniosła tyle już po 17-tej z 25 epok).

\subsubsection{Wydajność}

Wydajność każdej z metod klasyfikacji zmierzono na dwa sposoby. Pierwszym źródłem wyników był wykorzystany do badania dokładności skrypt \verb+image_test.py+, w którym zmierzono upływ czasu klasyfikacji wszystkich 14970 obrazów łącznie z ich obróbką przez ekstrator. Otrzymane wyniki są następujące:
\begin{itemize}
...
\begin{itemize}
    \item translacja kształtu,
    \item obrót kształtu,
    \item przechylenie kształtu,
    \item przybliżenie kształtu.
\end{itemize}

Wyniki wykonanych prób czasowych znajdują się w tabeli poniżej. Kolumny oznaczone jako \emph{geometric}, \emph{network (vec)} oraz \emph{network (conv)} odpowiadają odpowiednio klasyfikatorom: geometrycznemu, opartego na sieci neuronowej klasyfikującej wektory oraz na sieci konwolucyjnej.

\begin{table}[H]
    \begin{tabularx}{\textwidth}{|c|c|Y|Y|Y|}
        \hline
        \textbf{Test} & \textbf{Kształt} & \textbf{geometric} & \textbf{network (vec)} & \textbf{network (conv)} \\
        \hline
        \hline
        \multirow{4}{*}{\rotatebox[origin=c]{90}{\textbf{Translacja}}}
        & kwadrat & 00:14.368 & 00:14.368 & 00:14.368 \\
        \cline{2-5}
        & koło & 00:16.493 & 00:16.493 & 00:16.493 \\
        \cline{2-5}
        & trójkąt & 00:13.210 & 00:13.210 & 00:13.210 \\
        \cline{2-5}
        & gwiazda & 00:14.480 & 00:14.480 & 00:14.480 \\
        \hline
        \hline
        \multirow{4}{*}{\rotatebox[origin=c]{90}{\textbf{Obrót}}}
        & kwadrat & 00:17.291 & 00:17.291 & 00:17.291 \\
        \cline{2-5}
        & koło & 00:17.936 & 00:17.936 & 00:17.936 \\
        \cline{2-5}
        & trójkąt & 00:18.347 & 00:18.347 & 00:18.347 \\
        \cline{2-5}
        & gwiazda & 00:10.154 & 00:10.231 & 00:15.575 \\
        \hline
    \end{tabularx}
\end{table}
\begin{table}[H]
    \begin{tabularx}{\textwidth}{|c|c|Y|Y|Y|}
        ...
        & kwadrat & 00:03.113 & 00:06.254 & 00:06.285 \\
        \cline{2-5}
        & koło & 00:05.658 & 00:05.568 & 00:05.753 \\
        \cline{2-5}
        & trójkąt & 00:14.188 & 00:14.188 & 00:04.518 \\
        \cline{2-5}
        & gwiazda & 00:08.886 & 00:03.590 & 00:13.652 \\
        \hline
        \hline
        \multirow{4}{*}{\rotatebox[origin=c]{90}{\textbf{Przybliżenie}}}
        & kwadrat & 00:17.115 & 00:17.115 & 00:17.115 \\
        \cline{2-5}
        & koło & 00:08.984 & 00:09.150 & 00:08.921 \\
        \cline{2-5}
        & trójkąt & 00:04.483 & 00:15.498 & 00:15.498 \\
        \cline{2-5}
        & gwiazda & 00:04.558 & 00:04.590 & 00:14.308 \\
        \hline
    \end{tabularx}
